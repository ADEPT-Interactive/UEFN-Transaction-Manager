...
export async function normalizeImageToPowerOfTwo(imageBuffer: Buffer): Promise<Buffer> {
  if (imageBuffer.subarray(0, 10).toString('ascii') === '#?RADIANCE') {
    throw new Error('Radiance HDR images are not accepted for managed icons because HDR cannot carry the required RGBA alpha channel. Export or provide a PNG instead.');
  }
