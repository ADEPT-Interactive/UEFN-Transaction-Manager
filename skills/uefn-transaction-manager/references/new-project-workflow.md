...
8. Run the strongest safe session verification available. For any durable whose ownership affects live-session behavior, initialize external state from the generated reconciliation/authoritative ownership path and keep a persistent `Await<Stem>GrantedEvent()` path for same-session acquisition; use `Await<Stem>RemovedEvent()` only when the current contract and project semantics require it. Do not perform a real purchase. Audit that the managed file remains manager-owned and gameplay logic remains external.
