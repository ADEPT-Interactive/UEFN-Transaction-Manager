...
- After a generated managed file exists, treat `get_project_context.projectReadiness.transactionSetup` as an explicit operational gate. Confirm generated source and managed class presence, a placed and linked runtime device, current compile evidence, and `fullyOperational=true` before describing the transaction runtime as ready; follow its remediation when any state is missing or stale.
