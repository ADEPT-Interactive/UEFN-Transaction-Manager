...
test('4.0.1 distribution contract uses the ADEPT generic updater and stable human aliases', () => {
  ...
  assert.ok(readme.includes('UEFN-Entitlement-Manager-Setup.exe'));
  assert.ok(readme.includes('UEFN-Entitlement-Manager-Portable.zip'));
  assert.ok(userReadme.includes('UEFN-Entitlement-Manager-Setup.exe'));
...
test('release workflows separate draft human release assets from final R2 promotion', () => {
  ...
  assert.match(release, /UEFN-Entitlement-Manager-Setup\.exe/);
  assert.match(release, /UEFN-Entitlement-Manager-Portable\.zip/);
