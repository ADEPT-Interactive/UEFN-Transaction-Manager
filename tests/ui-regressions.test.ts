...
test('standalone startup paints before discovery and automatically installs its UEFN connector', () => {
  ...
  assert.match(read(path.join('electron', 'bootstrapPolicy.ts')), /maxAttempts = 3/);
  assert.match(read(path.join('electron', 'bootstrapPolicy.ts')), /bounded-bootstrap-attempts-exhausted/);
  ...
  assert.doesNotMatch(bridgeSource, /py import uefn_auto_connector/);
  assert.match(bridgeSource, /connectorAlive/);
  assert.match(bridgeSource, /method=uefn-python-console/);
  assert.match(nativeSource, /FindWindowW/);
  assert.match(nativeSource, /SendInput/);
  assert.match(nativeSource, /GetForegroundWindow/);
