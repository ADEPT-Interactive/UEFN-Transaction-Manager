...
if ($CatalogState -eq "initialized") {
    $managedData = '{"schemaVersion":4,"entitlements":[],"bundles":[],"storefrontMembership":{"allOffers":[],"focused":[]},"retiredVerseKeys":[]}'
    $managedDataBase64 = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($managedData))
    @(
        "# managed_transactions.verse"
        "# Generated by the isolated lifecycle harness for initialized-catalog coverage."
        "# UEFN_ENTITLEMENT_MANAGER_DATA_BEGIN"
        "# UEM_DATA $managedDataBase64"
        "# UEFN_ENTITLEMENT_MANAGER_DATA_END"
    ) | Set-Content -LiteralPath (Join-Path $contentRoot "managed_transactions.verse") -Encoding UTF8
}
