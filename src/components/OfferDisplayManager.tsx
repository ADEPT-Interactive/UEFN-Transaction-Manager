...
const emptyGroup = (groups: OfferDisplayGroup[], allocateVerseKey: (name: string) => string): OfferDisplayGroup => {
  ...
  triggerDeviceName: `${toPascalCase(verseKey)}Triggers`,
  });
};
...
const OfferDisplayEditor: React.FC<{
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" onMouseDown={event => { if (event.currentTarget === event.target) onClose(); }}><div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="offer-display-dialog-title" tabIndex={-1} className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-slate-700 bg-[#0d1326] p-6 shadow-2xl outline-none">
    ...
    <form onSubmit={event => { event.preventDefault(); if (!errors.length) onSave(form); }} className="space-y-4">
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-3"><label className="flex items-center justify-between gap-3 text-xs text-slate-300"><span><strong className="block text-white">Generate Trigger device array</strong><span className="text-[11px] text-slate-500">Lets beginners wire one or more Trigger devices without writing Verse.</span></span><input type="checkbox" checked={form.generateTriggerBinding} onChange={event => setForm({ ...form, generateTriggerBinding: event.target.checked })} className="accent-cyan-500" /></label>{form.generateTriggerBinding && <label className="mt-3 block text-xs text-slate-300">Trigger array name<input value={form.triggerDeviceName ?? ''} onChange={event => setForm({ ...form, triggerDeviceName: sanitizeVerseIdentifier(event.target.value) })} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 font-mono" /></label>}</div>
