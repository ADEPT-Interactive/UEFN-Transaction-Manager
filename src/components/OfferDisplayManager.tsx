...
import { toPascalCase, validateOfferDisplayGroup } from '../services/validator';
    ...
    <section className="space-y-3" aria-labelledby="offer-display-heading">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400">Choose the concrete offers shown in All Offers and each storefront. Primary and alternate offers are separate entries.</p>
        </div>
        <button type="button" onClick={() => setEditing(emptyGroup(membership.focused, allocateVerseKey))} className="flex items-center gap-1.5 rounded-xl border border-slate-700 bg-slate-800 px-3 py-2 text-xs font-bold hover:bg-slate-700"><Plus className="h-3.5 w-3.5" /> Add Storefront</button>
      </div>
      ...
      <ConfirmDialog open={Boolean(pendingDelete)} title={`Delete ${pendingDelete?.name ?? 'storefront'}?`} description={<>This removes only the storefront definition. Offers remain in All Offers and the catalog, and the project file remains unchanged until you save.</>} confirmLabel="Delete storefront" onCancel={() => setPendingDelete(null)} onConfirm={() => { if (pendingDelete) onChange({ ...membership, focused: membership.focused.filter(candidate => candidate.id !== pendingDelete.id) }); setPendingDelete(null); }} />
    </section>
  );
};
...
const OfferDisplayEditor: React.FC<{
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" onMouseDown={event => { if (event.currentTarget === event.target) requestClose(); }}><div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="offer-display-dialog-title" tabIndex={-1} className="flex min-h-0 max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-3xl border border-slate-700 bg-[#0d1326] shadow-2xl outline-none">
    <div className="flex shrink-0 items-center justify-between border-b border-slate-800 px-6 py-4"><div><p className="text-xs font-bold uppercase tracking-wider text-cyan-300">{isAllOffers ? 'Global storefront' : 'Storefront'}</p><h2 id="offer-display-dialog-title" className="mt-1 font-bold text-white">{isAllOffers ? 'Edit All Offers membership' : groups.some(candidate => candidate.id === form.id) ? 'Edit storefront' : 'Create storefront'}</h2></div><button type="button" aria-label="Close storefront editor" onClick={requestClose} className="p-2 text-slate-400 hover:text-white"><X className="h-5 w-5" /></button></div>
    <form onSubmit={event => { event.preventDefault(); commitForm(); }} className="flex min-h-0 flex-1 flex-col">
      <div className="min-h-0 flex-1 overflow-y-auto p-6 space-y-4">
        {!isAllOffers && <div className="grid gap-3"><label className="text-xs text-slate-300">Storefront title (up to {MARKETPLACE_CONSTRAINTS.nameMaxCharacters} characters)<input value={form.name} onChange={event => updateKeyFromName(event.target.value)} placeholder="Coin Store" className="utm-native-field mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2" /></label></div>}
