  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" onMouseDown={event => { if (event.currentTarget === event.target) requestClose(); }}><div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="offer-display-dialog-title" tabIndex={-1} className="flex min-h-0 max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-3xl border border-slate-700 bg-[#0d1326] shadow-2xl outline-none">
    ...
    <form onSubmit={event => { event.preventDefault(); commitForm(); }} className="flex min-h-0 flex-1 flex-col">
      <div className="min-h-0 flex-1 overflow-y-auto p-6 space-y-4">
        {!isAllOffers && <div className="grid gap-3"><label className="text-xs text-slate-300">Storefront title (up to {MARKETPLACE_CONSTRAINTS.nameMaxCharacters} characters)<input value={form.name} onChange={event => updateKeyFromName(event.target.value)} placeholder="Coin Store" className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2" /></label></div>}
