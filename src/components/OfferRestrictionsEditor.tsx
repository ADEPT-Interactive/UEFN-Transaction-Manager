...
const Flag: React.FC<{ code: string }> = ({ code }) => {
  ...
  // The source atlas is authored at 2x: each 48x36 flag occupies a 52x40
  // cell with a 2px source gutter. The renderer uses the matching 50% CSS
  // scale, so each cell is 26x20 and its gutter is one CSS pixel.
  const cellWidth = 26;
  const cellHeight = 20;
  const gutter = 1;
...
export const OfferRestrictionsEditor: React.FC<OfferRestrictionsEditorProps> = ({ restrictions, onChange, compact = false }) => {
  ...
  useClickAway(countryPickerRef, countryPickerOpen, () => setCountryPickerOpen(false));
  const closeCountryPickerFromKeyboard = (event: React.KeyboardEvent<HTMLDivElement>) => {
    if (event.key !== 'Escape' || !countryPickerOpen) return;
  ...
  return (
    <div className={`space-y-3 ${compact ? 'rounded-xl border border-slate-800 bg-slate-950/40 p-3' : ''}`}>
      ...
      <div ref={countryPickerRef} className="relative" onKeyDownCapture={closeCountryPickerFromKeyboard}>
        ...
        {countryPickerOpen && <div className="absolute left-0 right-0 top-full z-30 mt-2 rounded-xl border border-slate-700 bg-[#0d1326] p-2 shadow-2xl"><input autoFocus value={countrySearch} onChange={event => setCountrySearch(event.target.value)} placeholder={'e.g. "United States" or "US"'} className="utm-native-field w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-xs text-white" /><div className="mt-2 max-h-56 overflow-y-auto space-y-0.5">{filteredCountries.length > 0 ? filteredCountries.map(country => <button key={country.code} type="button" onClick={() => addCountry(country.code)} className="w-full flex items-center gap-2 rounded-lg px-2 py-1.5 text-left text-xs text-slate-300 hover:bg-cyan-500/10"><Flag code={country.code} /><span className="flex-1">{country.name}</span><span className="font-mono text-[10px] text-slate-500">{country.code}</span><Plus className="w-3 h-3 text-cyan-300" aria-hidden="true" /></button>) : <p className="px-2 py-3 text-xs text-slate-500">No matching countries.</p>}</div></div>}
      </div>
    </div>
  );
};
