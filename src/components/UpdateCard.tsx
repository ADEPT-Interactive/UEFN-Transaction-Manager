...
export const UpdateCard: React.FC<UpdateCardProps> = ({ state, onCheck, onDownload, onInstall, onLater }) => {
  ...
  if (state.status === 'up-to-date') {
    return <section className="mx-auto mb-5 flex max-w-6xl items-center justify-between gap-3 rounded-2xl border border-emerald-500/25 bg-emerald-500/5 px-4 py-3 text-xs text-emerald-200" role="status"><span className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4" />Transaction Manager is up to date ({state.currentVersion}).</span><button type="button" onClick={onLater} className="text-emerald-300/70 hover:text-emerald-100" aria-label="Dismiss update status"><X className="h-4 w-4" /></button></section>;
  }
  if (state.status === 'checking') return <section className="mx-auto mb-5 flex max-w-6xl items-center gap-2 rounded-2xl border border-slate-700 bg-slate-900/70 px-4 py-3 text-xs text-slate-300" role="status"><RefreshCw className="h-4 w-4 animate-spin text-cyan-300" />Checking for Transaction Manager updates…</section>;
  if (state.status === 'error') return <section className="mx-auto mb-5 flex items-center justify-between gap-3 rounded-2xl border border-amber-500/25 bg-amber-500/5 px-4 py-3 text-xs text-amber-200" role="alert"><span>{state.message ?? 'The update check failed. Transaction Manager is still ready to use.'}</span><button type="button" onClick={onCheck} className="font-bold text-cyan-300 hover:text-cyan-200">Try again</button></section>;

  const downloaded = state.status === 'downloaded';
  const downloading = state.status === 'downloading';
  if (state.dismissed && !downloaded && !downloading) return null;
  return <section className="mx-auto mb-5 max-w-6xl rounded-2xl border border-cyan-500/30 bg-cyan-500/[0.07] p-4 shadow-lg shadow-cyan-950/10" aria-label="Transaction Manager update">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div className="min-w-0">
        <p className="text-sm font-extrabold text-white">{downloaded ? 'Transaction Manager is ready to restart' : downloading ? 'Downloading the Transaction Manager update' : `Transaction Manager ${state.availableVersion ?? 'update'} is available`}</p>
