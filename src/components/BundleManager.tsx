    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onMouseDown={event => { if (event.currentTarget === event.target) requestClose(); }}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="bundle-dialog-title" tabIndex={-1} className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl border border-slate-700 bg-[#0d1326] p-6 shadow-2xl outline-none">
        ...
        <form onSubmit={event => { event.preventDefault(); commitForm(); }} className="space-y-4">
          ...
          <fieldset className="rounded-2xl border border-cyan-500/20 bg-cyan-500/[0.04] p-4 space-y-3"><legend className="px-1 text-xs font-extrabold text-cyan-200">Runtime behavior</legend><label className="flex items-center justify-between gap-3 text-xs text-slate-300"><span><span className="block font-bold text-white">Price</span><span className="text-slate-400">Choose whether Verse supplies the player-specific price.</span></span><select aria-label="Bundle price behavior" value={form.dynamicOffer?.priceBehavior ?? 'fixed'} onChange={e => setForm({ ...form, dynamicOffer: e.target.value === 'runtime' ? { priceBehavior: 'runtime' } : undefined })} className="w-48 rounded-lg border border-slate-700 bg-slate-950 px-2 py-2 text-xs"><option value="fixed">Fixed in UTM</option><option value="runtime">Calculated by Verse</option></select></label><p className="text-[11px] leading-5 text-slate-400">Runtime values are calculated by your project Verse. UTM validates them before the Marketplace UI opens. Runtime bundles are direct-purchase only.</p></fieldset>
