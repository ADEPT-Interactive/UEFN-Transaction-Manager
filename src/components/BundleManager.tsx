  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onMouseDown={event => { if (event.currentTarget === event.target) requestClose(); }}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="bundle-dialog-title" tabIndex={-1} className="flex min-h-0 w-full max-w-2xl max-h-[90vh] flex-col overflow-hidden rounded-3xl border border-slate-700 bg-[#0d1326] shadow-2xl outline-none">
        <div className="flex shrink-0 items-center justify-between border-b border-slate-800 px-6 py-4"><h2 id="bundle-dialog-title" className="font-bold text-white">{isExisting ? 'Edit bundle offer' : 'Create bundle offer'}</h2><button type="button" aria-label="Close bundle editor" onClick={requestClose} className="p-2 text-slate-400 hover:text-white"><X className="w-5 h-5" /></button></div>
        <form onSubmit={event => { event.preventDefault(); commitForm(); }} className="flex min-h-0 flex-1 flex-col">
          <div className="min-h-0 flex-1 overflow-y-auto p-6 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <label className="text-xs text-slate-300">Display name<input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="utm-native-field mt-1 w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2" /></label>
            <label className="text-xs text-slate-300 sm:col-span-2">Short description (up to {MARKETPLACE_CONSTRAINTS.shortDescriptionMaxCharacters} characters)<input value={form.shortDescription} onChange={e => setForm({ ...form, shortDescription: e.target.value })} className="utm-native-field mt-1 w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2" /></label>
            <label className="text-xs text-slate-300 sm:col-span-2">Description (up to {MARKETPLACE_CONSTRAINTS.descriptionMaxCharacters} characters before generated disclosures)<span className="utm-native-textarea-shell mt-1 block rounded-lg border border-slate-700 bg-slate-950"><textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} className="utm-native-field block w-full resize-none overflow-x-hidden overflow-y-auto border-0 bg-transparent px-3 py-2" /></span></label>
            ...
            <label className="text-xs text-slate-300">Texture expression<input value={form.iconTexture} onChange={e => setForm({ ...form, iconTexture: e.target.value })} className="utm-native-field mt-1 w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 font-mono" /></label>
            <label className="text-xs text-slate-300 sm:col-span-2">Duration disclosure<input value={form.durationDescription ?? ''} onChange={e => setForm({ ...form, durationDescription: e.target.value })} placeholder="e.g. Lasts 7 days after purchase" className="utm-native-field mt-1 w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2" /></label>
          ...
          <div className="flex shrink-0 justify-end gap-2 border-t border-slate-800 bg-slate-900/50 px-6 py-4"><button type="button" onClick={requestClose} className="px-4 py-2 text-xs">Cancel</button><button type="submit" disabled={errors.length > 0} className="px-4 py-2 rounded-xl text-xs font-bold bg-cyan-500 text-slate-950 disabled:opacity-50">Save bundle</button></div>
        </form>
      </div>
