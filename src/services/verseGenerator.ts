...
function manifestLines(entitlements: EntitlementItem[], bundles: BundleOffer[], offerDisplayGroups: OfferDisplayGroup[], retiredVerseKeys: string[]): string {
  const encoded = encodeBase64Utf8(JSON.stringify(cleanManagedData(entitlements, bundles, offerDisplayGroups, retiredVerseKeys)));
