...
export function generateVerseCode(
    ...
    push(
      ...
      ...(item.itemType === 'consumable'
        ? [
          ...
          `    # Each request stores (id, authoritative quantity still expected, native success, matched quantity awaiting success).`,
          `    var ${pascal}_PendingConsumeIntents:[player][]tuple(int, int, logic, int) = map{}`,
  ...
  push(
    ...
    '    # Consumed is correlated to a pending generated consume intent and emitted only after native success and a matching authoritative negative entitlement delta.',
  ...
  for (const item of entitlements) {
    ...
    if (item.itemType === 'consumable') {
      push(
        ...
        '        var Pending:[]tuple(int, int, logic, int) = array{}',
        `        if (Current := ${pascal}_PendingConsumeIntents[Player]):`,
        '            set Pending = Current',
        '        set Pending += array{(NextId, Quantity, false, 0)}',
        ...
        '            for (Request : Pending):',
        '                RequestId := Request(0)',
        '                var RequestedRemaining:int = Request(1)',
        '                OperationSucceeded := Request(2)',
        '                var BufferedMatched:int = Request(3)',
        '                if (Remaining > 0):',
        '                    MatchedNow := if (RequestedRemaining < Remaining) then RequestedRemaining else Remaining',
        ...
        `            if (set ${pascal}_PendingConsumeIntents[Player] = Updated) {}`,
        '',
        `    Confirm${pascal}ConsumeIntent(Player:player, RequestId:int):void =`,
        '        var Updated:[]tuple(int, int, logic, int) = array{}',
        `        if (Pending := ${pascal}_PendingConsumeIntents[Player]):`,
        '            for (Request : Pending):',
